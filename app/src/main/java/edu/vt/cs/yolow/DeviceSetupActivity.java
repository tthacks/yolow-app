...
import edu.vt.cs.yolow.ui.settings.SettingsFragment.FragmentSettings;
...
public class DeviceSetupActivity extends AppCompatActivity implements ServiceConnection, FragmentSettings {
    ...
    public void onServiceConnected(ComponentName name, IBinder service) {
        ...
        metawear.onUnexpectedDisconnect(status -> {
            ...
            metawear.connectAsync().continueWithTask(task -> task.isCancelled() || !task.isFaulted() ? task : MainContainer.reconnect(metawear))
                    .continueWith((Continuation<Void, Void>) task -> {
                        if (!task.isCancelled()) {
                            runOnUiThread(() -> {
                                ...
                                ((SettingsFragment) getSupportFragmentManager().findFragmentById(R.id.device_setup_fragment)).reconnected();
                            });
                        } else {
                            finish();
                        }
